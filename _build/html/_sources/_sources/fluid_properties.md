...
In other words, the viscosity $\mu$ is defined for Newtonian fluids (those for which the stress is proportional to the rate of strain) as the constant of proportionality between the shear stress $\tau$ and the velocity gradient $\frac{du}{dy}$ in a simple shear flow:
